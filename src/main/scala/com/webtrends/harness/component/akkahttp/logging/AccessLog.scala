...
object AccessLog  {
  ...
  def logAccess(request: AkkaHttpRequest, accessLogIdGetter: Option[AkkaHttpRequest => String], statusCode: StatusCode) = {

    // modify the logback.xml file to write the "AccessLog" entries to a file without all of the prefix information
    accessLogIdGetter.foreach(getter => {
      try {
        val id: String = getter(request)
        ...
        */
        accessLog.info( s"""${AccessLog.host} - $id [$requestTime] "${request.method.value} ${request.path} ${request.protocol.value}" $status - $elapsedTime - $origin - $userAgent""")

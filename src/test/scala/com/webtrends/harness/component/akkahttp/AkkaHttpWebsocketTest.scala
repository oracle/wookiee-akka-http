...
class TestWebsocket extends AkkaHttpWebsocket {
  ...
  override def handleText(text: String, bean: CommandBean): Option[TextMessage] = {
...
class TestWebsocketStream extends AkkaHttpWebsocket {
  ...
  override def handleTextStream(text: Source[String, _], bean: CommandBean): Option[TextMessage] = {
  ...
  override def handleText(text: String, bean: CommandBean): Option[TextMessage] = None
    ...
    "be able to take websocket input" in {
      ...
      WS("/greeter/friend", wsClient.flow) ~> routes ~>
        check {
          ...
          wsClient.expectMessage(ByteString("Hello abcdef! var1: friend"))
